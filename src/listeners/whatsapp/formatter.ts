...
/**
 * Converts GitHub-flavored markdown into text that renders correctly in
 * WhatsApp using only the formatting syntax that WhatsApp officially
 * supports (see https://faq.whatsapp.com/539178204879377/):
 *
 *  - `*bold*`
 *  - `_italic_`
 *  - `~strikethrough~`
 ...
 *  - `1. numbered`
 *
 * Headings, links, images, tables, task lists and similar constructs have
 * no native equivalent in WhatsApp, so they gracefully degrade to the
 * closest supported representation (bold text, `text (url)` form, aligned
 * monospace blocks, `[x]` markers, etc.).
 */
export class WhatsAppFormatter implements IFormatter {
  ...
  private renderLink(node: Link): string {
    const text = this.renderInline(node.children).trim();
    return formatPlainLink(text, node.url, node.title ?? null);
  }

  private renderImage(node: Image): string {
    return formatPlainLink(node.alt ?? "", node.url, node.title ?? null);
  }
  ...
  private renderLinkReference(node: LinkReference): string {
    ...
    if (!def)
      return text.length > 0 ? text : `[${node.label ?? node.identifier}]`;
    return formatPlainLink(text, def.url, def.title);
  }
  ...
  private renderImageReference(node: ImageReference): string {
    ...
    const alt = node.alt ?? "";
    if (!def)
      return alt.length > 0 ? alt : `[${node.label ?? node.identifier}]`;
    return formatPlainLink(alt, def.url, def.title);
  }

  private renderFootnoteReference(node: FootnoteReference): string {
    const id = node.identifier.toLowerCase();
    if (!this.footnotes.has(id)) return `[^${node.identifier}]`;
