...
/**
 * Converts GitHub-flavored markdown into Telegram `MarkdownV2` text. The
 * output follows the escape rules documented at
 * https://core.telegram.org/bots/api#markdownv2-style:
 *
 *  - In normal text the 18 reserved characters
 *    `_`, `*`, `[`, `]`, `(`, `)`, `~`, `` ` ``, `>`, `#`, `+`, `-`,
 *    `=`, `|`, `{`, `}`, `.`, `!` and the backslash itself must be
 *    prefixed with `\`.
 *  - Inside `` ` `` / ```` ``` ```` entities only `` ` `` and `\` are
 *    escaped.
 *  - Inside the `(...)` part of inline links only `)` and `\` are escaped.
 *
 * Supported output constructs include `*bold*`, `_italic_`, `~strike~`,
 * `` `code` `` / fenced blocks (language tag preserved), `[text](url)`
 * inline links, `>quote` per line (nested supported), bullet lists with
 * `•`/`◦`, ordered lists with `N\. item`, escaped task list markers,
 * em-dash thematic breaks and tables degraded to aligned monospace blocks.
 */
export class TelegramFormatter implements IFormatter {
  ...
  private renderListItem(
    ...
    const continuation = " ".repeat([...bullet.replace(/\\/g, "")].length);
  ...
  private renderCode(node: Code): string {
    const body = escapeMarkdownV2Code(
      (node.value ?? "").replace(/\r\n?/g, "\n"),
    );
    const lang = typeof node.lang === "string" ? node.lang.trim() : "";
    const fenceHeader = lang.length > 0 ? `\`\`\`${lang}` : "```";
    return `${fenceHeader}\n${body}\n\`\`\``;
  }
  ...
  private renderTable(node: Table): string {
    const rows: string[][] = node.children.map((row: TableRow) =>
      row.children.map((cell) =>
        unescapeMarkdownV2(this.renderInline(cell.children))
          .replace(/\s+/g, " ")
          .trim(),
      ),
    );
    ...
    return `\`\`\`\n${escapeMarkdownV2Code(lines.join("\n"))}\n\`\`\``;
  }
  ...
  private renderHtml(raw: string): string {
    const stripped = raw
      .replace(/<!--[\s\S]*?-->/g, "")
      .replace(/<[^>]*>/g, "");
    return escapeMarkdownV2Text(stripped.trim());
  }
  ...
  private renderInlineNode(node: PhrasingContent): string {
    switch (node.type) {
      case "text":
        return escapeMarkdownV2Text((node as Text).value);
      case "strong": {
      ...
      case "inlineCode":
        return `\`${escapeMarkdownV2Code((node as InlineCode).value)}\``;
      case "break":
        return "\n";
      case "link":
  ...
  private renderLink(node: Link): string {
    const text = this.renderInline(node.children).trim();
    return formatMarkdownV2Link(text, node.url);
  }

  private renderImage(node: Image): string {
    return formatMarkdownV2Link(escapeMarkdownV2Text(node.alt ?? ""), node.url);
  }
  ...
  private renderLinkReference(node: LinkReference): string {
    ...
    if (!def) {
      return text.length > 0
        ? text
        : escapeMarkdownV2Text(`[${node.label ?? node.identifier}]`);
    }
    return formatMarkdownV2Link(text, def.url);
  }

  private renderImageReference(node: ImageReference): string {
    const def = this.definitions.get(node.identifier.toLowerCase());
    const alt = escapeMarkdownV2Text(node.alt ?? "");
    if (!def) {
      return alt.length > 0
        ? alt
        : escapeMarkdownV2Text(`[${node.label ?? node.identifier}]`);
    }
    return formatMarkdownV2Link(alt, def.url);
  }
  ...
  private renderFootnoteReference(node: FootnoteReference): string {
    ...
    if (!this.footnotes.has(id)) {
      return escapeMarkdownV2Text(`[^${node.identifier}]`);
    }
...
/** Escape the 18 MarkdownV2 reserved characters plus the backslash. */
function escapeMarkdownV2Text(value: string): string {
  return value.replace(/[_*\[\]()~`>#+\-=|{}.!\\]/g, "\\$&");
}

/** Escape only `\` and `` ` `` — the rules that apply inside `pre`/`code`. */
function escapeMarkdownV2Code(value: string): string {
  return value.replace(/[\\`]/g, "\\$&");
}

/** Escape only `\` and `)` — the rules that apply inside inline-link URLs. */
function escapeMarkdownV2Url(value: string): string {
  return value.replace(/[\\)]/g, "\\$&");
}

function formatMarkdownV2Link(text: string, url: string): string {
  const cleanUrl = (url ?? "").trim();
  if (cleanUrl.length === 0) return text;
  const label = text.trim();
  const encodedUrl = escapeMarkdownV2Url(cleanUrl);
  if (label.length === 0) {
    return `[${escapeMarkdownV2Text(cleanUrl)}](${encodedUrl})`;
  }
  return `[${label}](${encodedUrl})`;
}

/** Drop MarkdownV2 escape backslashes so a value can appear in a code block. */
